          ...
          <div className="split-grid" style={{ alignItems: "center" }}>
            ...
            <div>
              ...
              <h1 className="display-lg fade-up-d1" style={{ marginBottom: 24 }}>
                Hire on evidence,<br />
                <span className="teal-gradient">not instinct.</span>
              </h1>
              ...
              <div className="fade-up-d3" style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 52 }}>
                <Link href="/contact" className="btn-primary" style={{ animation: "glowRing 3s ease-in-out infinite" }}>
                  Start free trial →
                </Link>
          ...
          <div className="col-3-grid">
            {[
              ...
              <ScrollReveal key={p.title} direction="scale" delay={i * 100} threshold={0.06}>
                <div style={{ background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.07)", borderRadius: 20, padding: "32px 28px", height: "100%", position: "relative", overflow: "hidden", transition: "border-color 0.3s, transform 0.3s", cursor: "default" }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = `${p.color}33`; (e.currentTarget as HTMLElement).style.transform = "translateY(-8px)"; }}
              ...
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  {integrityFeatures.map((item, i) => (
                    <div key={item.label} style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(0,201,167,0.05)", border: "0.5px solid rgba(0,201,167,0.12)", borderRadius: 10, padding: "12px 14px", transition: "all 0.3s", cursor: "default", animationDelay: `${i * 0.1}s` }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "rgba(0,201,167,0.1)"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(0,201,167,0.3)"; (e.currentTarget as HTMLElement).style.transform = "scale(1.03)"; }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "rgba(0,201,167,0.05)"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(0,201,167,0.12)"; (e.currentTarget as HTMLElement).style.transform = "scale(1)"; }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 32 }}>
                  {["Who scored the highest overall?", "Which candidates expect under Rs. 80,000?", "Summarise the top 3 candidates.", "Who flagged integrity issues?"].map((q, i) => (
                    <div key={q} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 16px", background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.07)", borderRadius: 10, fontSize: "0.87rem", color: "var(--gray-2)", transition: "all 0.25s", cursor: "default", animationDelay: `${i * 0.1}s` }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(0,201,167,0.3)"; (e.currentTarget as HTMLElement).style.background = "rgba(0,201,167,0.05)"; (e.currentTarget as HTMLElement).style.transform = "translateX(6px)"; }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(255,255,255,0.07)"; (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.03)"; (e.currentTarget as HTMLElement).style.transform = "translateX(0)"; }}>
...
function StatCounter({ target, suffix, label, prefix, active }: { target: number; suffix: string; label: string; prefix: string; active: boolean }) {
  ...
  return (
    <div style={{ animation: active ? "countUp 0.6s ease-out both" : "none" }}>
      <p style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "1.6rem", color: "var(--teal)", letterSpacing: "-0.04em", lineHeight: 1 }}>
        {prefix}{val}{suffix}
      </p>
...
function FeatureCard({ f }: { f: { icon: string; title: string; desc: string } }) {
  return (
    <div
      style={{ background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.07)", borderRadius: 20, padding: "28px", height: "100%", position: "relative", overflow: "hidden", transition: "all 0.35s cubic-bezier(0.16,1,0.3,1)", cursor: "default" }}
      onMouseEnter={e => {
        const el = e.currentTarget as HTMLElement;
        el.style.borderColor = "rgba(0,201,167,0.25)";
        el.style.transform = "translateY(-8px)";
        el.style.boxShadow = "0 24px 60px rgba(0,0,0,0.4), 0 0 0 0.5px rgba(0,201,167,0.1)";
    ...
    >
      ...
      {/* top gradient line */}
      <div style={{ position: "absolute", top: 0, left: "15%", right: "15%", height: 1, background: "linear-gradient(90deg, transparent, rgba(0,201,167,0.5), transparent)", opacity: 0, transition: "opacity 0.35s" }} className="top-line" />
      <div style={{ width: 46, height: 46, borderRadius: 12, background: "rgba(0,201,167,0.08)", border: "1px solid rgba(0,201,167,0.14)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 18, fontSize: "22px", transition: "all 0.3s" }}>{f.icon}</div>
...
function StepMockup({ index }: { index: number }) {
  if (index === 0) return <CreateRoleMockup />;
  if (index === 1) return <AiGeneratorMockup />;
  if (index === 2) return <InviteMockup />;
  if (index === 3) return <AssessmentMockup />;
...
function PricingCard({ plan }: { plan: typeof pricingPlans[0] }) {
  ...
  return (
    ...
    >
      {plan.highlight && (
        <div style={{ position: "absolute", top: -13, left: "50%", transform: "translateX(-50%)", background: "var(--teal)", color: "var(--navy)", fontSize: "0.66rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "4px 16px", borderRadius: 100, whiteSpace: "nowrap", zIndex: 2 }}>Most popular</div>
      )}
      <div style={{ background: plan.highlight ? "var(--navy-2)" : "rgba(255,255,255,0.03)", border: plan.highlight ? "none" : `0.5px solid ${hovered ? "rgba(0,201,167,0.2)" : "rgba(255,255,255,0.08)"}`, borderRadius: plan.highlight ? 26 : 28, padding: "36px 30px", height: "100%", display: "flex", flexDirection: "column", transition: "all 0.3s", transform: hovered ? "translateY(-6px)" : "none", boxShadow: hovered ? "0 28px 72px rgba(0,0,0,0.45)" : "none" }}>
      ...
      <div style={{ padding: 18 }}>
        ...
        <div style={{ background: "rgba(0,201,167,0.06)", border: "0.5px solid rgba(0,201,167,0.18)", borderRadius: 12, padding: "12px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {[0, 1, 2].map(d => (
              <div key={d} style={{ width: 5, height: 5, borderRadius: "50%", background: "var(--teal)", opacity: 0.6, animation: `typingDots 1.4s ease-in-out infinite`, animationDelay: `${d * 0.2}s` }} />
            ))}
          </div>
        </div>
      </div>
    ...
    <div style={{ background: "rgba(8,17,31,0.97)", border: "1px solid rgba(245,158,11,0.18)", borderRadius: 20, padding: 22, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 18 }}>
        {[
          ...
          <div key={e.time} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", background: "rgba(255,255,255,0.02)", borderRadius: 9, border: `0.5px solid ${e.level === "alert" ? "rgba(239,68,68,0.2)" : e.level === "warn" ? "rgba(245,158,11,0.14)" : "rgba(255,255,255,0.05)"}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ width: 6, height: 6, borderRadius: "50%", background: e.level === "alert" ? "#ef4444" : e.level === "warn" ? "#f59e0b" : "var(--teal)", flexShrink: 0 }} />
...
function AiChatMockup() {
  return (
    <div style={{ background: "rgba(255,255,255,0.04)", border: "0.5px solid rgba(255,255,255,0.1)", borderRadius: 20, overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "15px 18px", borderBottom: "0.5px solid rgba(255,255,255,0.07)", background: "rgba(0,201,167,0.03)" }}>
        <div style={{ width: 30, height: 30, borderRadius: 9, background: "rgba(0,201,167,0.14)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "15px" }}>✨</div>
        <div>
      ...
      <div style={{ padding: "12px 14px", borderTop: "0.5px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 9 }}>
        ...
        <div style={{ width: 34, height: 34, borderRadius: 9, background: "var(--teal)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "15px", cursor: "pointer", transition: "transform 0.2s" }}>→</div>
      </div>
    </div>
  );
}
